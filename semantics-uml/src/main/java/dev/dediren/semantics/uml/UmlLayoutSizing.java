...
/**
 * UML node sizing: relocated verbatim from the UML branches of the old single generic-graph {@code
 * GenericGraphLayoutSizing} (Plan B P3). Covers compact activity nodes (32x32), sequence-diagram
 * geometry (interaction/lifeline/execution/gate/destruction), state-machine vertices, use-case
 * actors/cases/extension points, component ports, deployment nodes, and label/compartment-driven
 * structural classifier sizing (class/interface/data type/enumeration); every other UML node type
 * falls back to the shared 160x80 floor.
 */
public final class UmlLayoutSizing {
  ...
  private static double umlStructuralWidthHint(SourceNode sourceNode) {
    ...
    int maxChars =
        umlClassifierLineLengths(sourceNode.type(), sourceNode.label(), properties).stream()
            .max(Comparator.naturalOrder())
            .orElse(sourceNode.label().length());
    return roundUp(
        Math.max(
            maxChars * UML_TEXT_CHAR_WIDTH + UML_TEXT_HORIZONTAL_PADDING, UML_STRUCTURAL_MIN_WIDTH),
        20.0);
  }
  ...
  private static double umlStructuralHeightHint(SourceNode sourceNode) {
    ...
    int attributeCount =
        sourceNode.type().equals("Enumeration")
            ? umlArrayLen(properties, "literals")
            : umlArrayLen(properties, "attributes");
    int operationCount =
        sourceNode.type().equals("Enumeration") ? 0 : umlArrayLen(properties, "operations");
    ...
    return roundUp(
        Math.max(
            titleHeight
                ...
                + operationExtra,
            UML_STRUCTURAL_MIN_HEIGHT),
        10.0);
  }

  private static List<Integer> umlClassifierLineLengths(
      String nodeType, String label, JsonNode properties) {
    ...
    if (nodeType.equals("Enumeration")) {
      ...
      umlArrayValues(properties, "attributes").stream()
          .map(UmlLayoutSizing::umlAttributeLine)
          .map(String::length)
          .forEach(lengths::add);
      umlArrayValues(properties, "operations").stream()
          .map(UmlLayoutSizing::umlOperationLine)
          .map(String::length)
          .forEach(lengths::add);
    }
    return lengths;
  ...
  private static String umlAttributeLine(JsonNode attribute) {
    ...
    return attributeType.isEmpty()
        ? visibility + " " + name
        : visibility + " " + name + " : " + attributeType;
  }
  ...
  private static String umlOperationLine(JsonNode operation) {
    ...
    String parameters =
        umlArrayValues(operation, "parameters").stream()
            .map(UmlLayoutSizing::umlParameterText)
