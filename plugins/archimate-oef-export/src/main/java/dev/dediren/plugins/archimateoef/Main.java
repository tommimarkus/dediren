...
public final class Main {
    ...
    private static final String OEF_SCHEMA = "http://www.opengroup.org/xsd/archimate/3.1/archimate3_Model.xsd";
    ...
    private static final List<String> OFFICIAL_OEF_SCHEMA_FILES = List.of(
    ...
    private static void validateArchimateTypes(ExportRequest request)
        ...
        for (int relationshipIndex = 0; relationshipIndex < request.source().relationships().size(); relationshipIndex++) {
            var relationship = request.source().relationships().get(relationshipIndex);
            Archimate.validateRelationshipType(
                    relationship.type(),
                    "$.source.relationships[" + relationshipIndex + "].type");
    ...
    private static void validateArchimateJunctionSemantics(ExportRequest request)
        ...
        for (int index = 0; index < request.source().nodes().size(); index++) {
            ...
            nodes.add(new JunctionValidationNode(node.id(), node.type(), "$.source.nodes[" + index + "]"));
        }
        var relationships = request.source().relationships().stream()
    ...
    private static String buildOef(ExportRequest request, OefExportPolicy policy) {
        ...
        request.source().relationships().forEach(relationship ->
                relationshipIds.put(relationship.id(), ids.oefId("rel", relationship.id())));
        var sourceNodesById = request.source().nodes().stream()
                .collect(Collectors.toMap(SourceNode::id, node -> node));
        var semanticGroups = request.layoutResult().groups().stream()
                .filter(group -> {
                    String sourceId = semanticGroupSourceId(group);
        ...
        request.layoutResult().nodes().forEach(node ->
                viewNodeIds.put(node.id(), ids.oefId("vn-" + request.layoutResult().viewId(), node.id())));
        var groupViewNodeIds = new HashMap<String, String>();
        semanticGroups.forEach(group ->
                groupViewNodeIds.put(group.id(), ids.oefId("vg-" + request.layoutResult().viewId(), group.id())));
        var viewConnectionIds = new HashMap<String, String>();
        request.layoutResult().edges().forEach(edge ->
                viewConnectionIds.put(edge.id(), ids.oefId("vc-" + request.layoutResult().viewId(), edge.id())));

        StringBuilder xml = new StringBuilder();
        xml.append("<model xmlns=\"").append(OEF_NS).append("\" xmlns:xsi=\"").append(XSI_NS)
                .append("\" xsi:schemaLocation=\"").append(OEF_NS).append(" ").append(OEF_SCHEMA)
                .append("\" identifier=\"").append(attr(policy.modelIdentifier())).append("\">");
        writeTextElement(xml, "name", policy.modelName());
        xml.append("<elements>");
        for (var node : request.source().nodes()) {
            xml.append("<element identifier=\"").append(attr(elementIds.get(node.id())))
                    .append("\" xsi:type=\"").append(attr(node.type())).append("\">");
        ...
        for (var relationship : request.source().relationships()) {
            xml.append("<relationship identifier=\"").append(attr(relationshipIds.get(relationship.id())))
                    .append("\" source=\"").append(attr(elementIds.get(relationship.source())))
                    .append("\" target=\"").append(attr(elementIds.get(relationship.target())))
                    .append("\" xsi:type=\"").append(attr(relationship.type())).append("\">");
            writeTextElement(xml, "name", relationship.label());
            xml.append("</relationship>");
        }
        xml.append("</relationships>");

        xml.append("<views><diagrams><view identifier=\"").append(attr(policy.viewIdentifier()))
                .append("\" xsi:type=\"Diagram\" viewpoint=\"").append(attr(policy.viewpoint())).append("\">");
        writeTextElement(xml, "name", policy.viewName());
        for (var group : semanticGroups) {
            String sourceId = semanticGroupSourceId(group);
            xml.append("<node identifier=\"").append(attr(groupViewNodeIds.get(group.id())))
                    .append("\" xsi:type=\"Element\" elementRef=\"").append(attr(elementIds.get(sourceId)))
                    .append("\" x=\"").append(formatNumber(group.x()))
                    .append("\" y=\"").append(formatNumber(group.y()))
                    .append("\" w=\"").append(formatNumber(group.width()))
                    .append("\" h=\"").append(formatNumber(group.height()))
                    .append("\"/>");
        }
        for (var node : request.layoutResult().nodes()) {
            xml.append("<node identifier=\"").append(attr(viewNodeIds.get(node.id())))
                    .append("\" xsi:type=\"Element\" elementRef=\"").append(attr(elementIds.get(node.sourceId())))
                    .append("\" x=\"").append(formatNumber(node.x()))
                    .append("\" y=\"").append(formatNumber(node.y()))
                    .append("\" w=\"").append(formatNumber(node.width()))
                    .append("\" h=\"").append(formatNumber(node.height()))
                    .append("\"/>");
        }
    ...
    private static Path resolveOfficialOefSchemaDir(Map<String, String> env)
        ...
        Path schemaDir;
        try {
            schemaDir = SchemaCacheModule.schemaCacheBaseDir(env, SCHEMA_CACHE_DIR_ENV, OEF_SCHEMA_DIR_ENV)
                    .resolve("opengroup")
                    .resolve("archimate")
                    .resolve("3.1");
        ...
        for (String fileName : OFFICIAL_OEF_SCHEMA_FILES) {
            ...
            if (!SchemaCacheModule.isNonEmptyFile(schemaPath)) {
                throw new OefSchemaValidationException(
                        ...
                        "official OEF schema file " + schemaPath
                                + " is missing or empty; provide all ArchiMate 3.1 OEF XSD files or unset "
                                + OEF_SCHEMA_DIR_ENV + " to allow cache download");
            }
        }
    }
    ...
    private static void writeTextElement(StringBuilder xml, String name, String text) {
        xml.append("<").append(name).append(" xml:lang=\"en\">")
                .append(text(text))
