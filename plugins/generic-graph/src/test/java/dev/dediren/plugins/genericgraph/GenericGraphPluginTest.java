...
class GenericGraphPluginTest {
  ...
  void projectsUmlRenderMetadata() throws Exception {
    ...
    assertThat(data.at("/nodes/enum-order-status/properties/literals/1").asText())
        .isEqualTo("Submitted");
    assertThat(data.at("/edges/order-has-lines/type").asText()).isEqualTo("Composition");
    assertThat(data.at("/edges/order-has-lines/properties/source_multiplicity").asText())
        .isEqualTo("1");
    assertThat(data.at("/edges/order-has-lines/properties/target_multiplicity").asText())
        .isEqualTo("1..*");
  ...
  void projectsJunctionRoleOntoArchimateLayoutNodes() throws Exception {
    ...
    assertThat(layoutRequestNode(data, "fulfillment-junction").at("/role").asText())
        .isEqualTo("junction");
    assertThat(layoutRequestNode(data, "approval-junction").at("/role").asText())
        .isEqualTo("junction");
  ...
  void projectsUmlStateMachineRenderMetadata() throws Exception {
    ...
    assertThat(data.at("/edges/t-approve/properties/guard").asText())
  ...
  void projectsUmlUseCaseRenderMetadata() throws Exception {
    ...
    assertThat(data.at("/nodes/place-order/properties/subject").asText())
        .isEqualTo("order-service");
    assertThat(data.at("/nodes/payment-extension/type").asText()).isEqualTo("ExtensionPoint");
    assertThat(data.at("/nodes/payment-extension/properties/use_case").asText())
  ...
  void projectsUmlComponentRenderMetadata() throws Exception {
    ...
    assertThat(data.at("/nodes/port-rest-api/properties/component").asText())
        .isEqualTo("component-order-api");
    assertThat(data.at("/nodes/port-rest-api/properties/provided/0").asText())
        .isEqualTo("interface-order-api");
    ...
    assertThat(data.at("/edges/order-api-realizes-order-api/type").asText())
  ...
  void projectsUmlDeploymentRenderMetadata() throws Exception {
    ...
    assertThat(data.at("/nodes/ee-orders-runtime/properties/node").asText())
        .isEqualTo("device-prod-node");
    assertThat(data.at("/nodes/artifact-orders-service/type").asText()).isEqualTo("Artifact");
    assertThat(data.at("/nodes/deployment-spec-orders/type").asText())
        .isEqualTo("DeploymentSpecification");
    assertThat(data.at("/edges/deploy-orders-service/type").asText()).isEqualTo("Deployment");
    assertThat(data.at("/edges/artifact-manifests-order-api/type").asText())
        .isEqualTo("Manifestation");
    assertThat(data.at("/edges/orders-runtime-payment-path/type").asText())
        .isEqualTo("CommunicationPath");
  ...
  void projectsUmlSequenceEdgeRenderMetadata() throws Exception {
    ...
    assertThat(data.at("/edges/m1/properties/interaction").asText())
  ...
  void projectsUmlSequenceFragmentRenderMetadata() throws Exception {
    PluginResult result =
        Main.executeForTesting(
            new String[] {
              "project", "--target", "render-metadata", "--view", "sequence-fragments-view"
            },
  ...
  void projectsUmlSequenceLayoutConstraints() throws Exception {
    ...
    assertThat(jsonTexts(lifelineOrder.get("subjects"))).containsExactly("customer", "service");
    assertThat(messageOrder.at("/id").asText())
        .isEqualTo("sequence-view.uml.sequence.message-order");
    assertThat(jsonTexts(messageOrder.get("subjects"))).containsExactly("m2", "m1", "m3");
    assertSchemaValid("schemas/layout-request.schema.json", data);
  }
  ...
  void projectsUmlSequenceMessageOrderWithLargeIntegralSequence() throws Exception {
    ...
    assertThat(jsonTexts(messageOrder.get("subjects"))).containsExactly("m3", "m2", "m1");
  }
            ...
            """
                {
                  "model_schema_version": "model.schema.v1",
                  "required_plugins": [
                    { "id": "generic-graph", "version": "2026.06.10" },
                    { "id": "archimate-oef", "version": "2026.06.10" }
                  ],
                  "nodes": [
                    { "id": "api", "type": "ApplicationComponent", "label": "API", "properties": {} },
  ...
  void rejectsArchimateJunctionWithInvalidEffectiveEndpoint() throws Exception {
    PluginResult result =
        Main.executeForTesting(
            new String[] {"validate", "--profile", "archimate"},
            archimateJunctionSource(
                "Realization", "Realization", "service", "junction", "component"));
  ...
  private static String sequenceFixtureWithAdditionalSequenceNodes() throws Exception {
    ...
    addSequenceNode(
        nodes, viewNodes, "service-destroyed", "DestructionOccurrenceSpecification", "");
  ...
  private static void addSequenceNode(
    ...
    node.putObject("properties").putObject("uml").put("interaction", "interaction-place-order");
    viewNodes.add(id);
  }
  ...
  private static String archimateJunctionSource(
    ...
    String targetType =
        targetId.equals("component") ? "ApplicationComponent" : "ApplicationService";
    return """
                {
                  "model_schema_version": "model.schema.v1",
                  "required_plugins": [
