...
/**
 * Envelope-shaped test harness for the semantics router: it parses stdin, delegates to a {@link
 * SemanticsRouterEngine} wired with all three notations, and shapes the command envelope so the
 * relocated generic-graph suites can drive the engine without a process boundary. Behaviour matches
 * the retired {@code dev.dediren.plugins.genericgraph.Main} test harness verbatim (same command
 * grammar, same exit codes, same error precedence); only the constructed engine changed from the
 * single {@code GenericGraphEngine} to the routed {@link SemanticsRouterEngine}.
 */
...
final class RouterHarness {
  ...
  private static SemanticsRouterEngine engine() {
  ...
  static RouterResult executeForTesting(String[] args, String stdin) throws Exception {
  ...
  private static int validateFromStdin(String[] args, InputStream stdin, PrintStream stdout)
    ...
    if (profile == null) {
      ...
      return printError(stdout, SemanticsRouterEngine.profileRequired());
    }
    SemanticsRouterEngine engine = engine();
