...
fn dist_build_serializes_parallel_invocations() {
    ...
    repo.block_notice_generation();
    let first = repo.run_xtask(["dist", "build"]);
    repo.wait_for_notice_generation();
    let second = repo.run_xtask(["dist", "build"]);
    repo.release_notice_generation();
...
fn dist_build_includes_agent_usage_docs() {
    ...
    assert!(
        metadata.get("elk_helper").is_none(),
        "bundle metadata should not advertise the removed ELK helper runtime"
    );
}
...
fn dist_build_includes_third_party_notices() {
    ...
    assert!(
        ...
        "dist build should use generated Rust notices, not a static source notice"
    );
    assert!(
        !notice.contains("ELK Java Helper Runtime Dependencies")
            && !notice.contains("dediren-elk-layout-java"),
        "third-party notices should not include removed Java helper notices"
    );
    ...
    assert!(
        !String::from_utf8_lossy(&archive_listing.stdout)
            .lines()
            .any(|path| path.starts_with(&format!("{bundle_name}/runtimes/elk-layout-java"))),
        "dist archive should not include the removed ELK helper runtime\nstdout:\n{}",
        String::from_utf8_lossy(&archive_listing.stdout),
    );
}
...
#[cfg(unix)]
#[test]
fn cargo_about_policy_accepts_pinned_elkrs_license() {
    let policy = fs::read_to_string(repo_file(".cargo/about.toml")).unwrap();

    assert!(
        policy.contains("\"EPL-2.0\""),
        "cargo-about policy should explicitly accept the pinned elkrs EPL-2.0 license"
    );
}
...
impl FakeDistRepo {
    ...
    fn block_notice_generation(&self) {
        fs::create_dir_all(self.root.path().join(".cache")).unwrap();
        fs::write(self.root.path().join(".cache/block-notice-generation"), "").unwrap();
    }

    fn wait_for_notice_generation(&self) {
        let marker = self.root.path().join(".cache/notice-generation-running");
        let deadline = Instant::now() + Duration::from_secs(5);
        while !marker.exists() {
            assert!(
                Instant::now() < deadline,
                "stub notice generation did not start"
            );
            std::thread::sleep(Duration::from_millis(10));
        }
    }

    fn release_notice_generation(&self) {
        fs::create_dir_all(self.root.path().join(".cache")).unwrap();
        fs::write(
            self.root.path().join(".cache/release-notice-generation"),
            "",
        )
        .unwrap();
    }
    ...
    fn write_smoke_archive(&self) -> PathBuf {
        ...
        self.write_executable_at(
            ...
            r#"#!/usr/bin/env bash
set -euo pipefail
for name in DEDIREN_PLUGIN_DIRS DEDIREN_PLUGIN_GENERIC_GRAPH DEDIREN_PLUGIN_ELK_LAYOUT DEDIREN_PLUGIN_SVG_RENDER DEDIREN_PLUGIN_ARCHIMATE_OEF DEDIREN_PLUGIN_UML_XMI DEDIREN_ELK_RESULT_FIXTURE; do
  if [[ -n "${!name:-}" ]]; then
...
if [[ "${1:-}" == "about" && "${2:-}" == "generate" ]]; then
  REPO_ROOT=$(pwd -P)
  if [[ -f "$REPO_ROOT/.cache/block-notice-generation" ]]; then
    MARKER="$REPO_ROOT/.cache/notice-generation-running"
    RELEASE="$REPO_ROOT/.cache/release-notice-generation"
    mkdir -p "$REPO_ROOT/.cache"
    if ! mkdir "$MARKER"; then
      echo "notice generation overlapped" >&2
      exit 77
    fi
