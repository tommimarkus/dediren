...
/**
 * Whole-reactor enforcement of the compile-time dependency spine in
 * docs/architecture-guidelines.md §2 (the allowed edge table and the
 * inward-only, acyclic rules). dist-tool is the only non-profile module whose
 * (transitive) test classpath sees every product module, so these rules are
 * asserted here in one place. Runs as a plain JUnit test against core ArchUnit
 * (no archunit-junit5 engine) for compatibility with this JUnit 6 build.
 */
class ArchitectureRulesTest {
    ...
    void contractsDependsOnNothingInternal() {
        noClasses().that().resideInAPackage(CONTRACTS)
            .should().dependOnClassesThat().resideInAnyPackage(
                ...
                "dev.dediren.tools..")
            .because("contracts is the stable foundation and must not depend on any"
    ...
    void pluginsDoNotDependOnCore() {
        noClasses().that().resideInAPackage(PLUGINS)
            .should().dependOnClassesThat().resideInAPackage(CORE)
            .because("plugins are reachable only across the process boundary; a compile"
    ...
    void coreDoesNotDependOnPlugins() {
        noClasses().that().resideInAPackage(CORE)
            ...
            .because("core discovers and runs plugins as subprocesses; it knows them"
    ...
    void cliDoesNotDependOnPlugins() {
        noClasses().that().resideInAPackage(CLI)
            .should().dependOnClassesThat().resideInAPackage(PLUGINS)
            .because("cli depends on core and contracts only; its plugin dependencies are"
