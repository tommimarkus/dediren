...
class ArchitectureRulesTest {
  ...
  void cliDependsOnNoNotationOrUtilityCore() {
    // §2 allows cli only contracts, core, engine-api and ir, plus mcp-server (the MCP adapter
    // edge) and the engine implementations (confined to EngineWiring by the rule below). The
    // notation cores were the one forbidden cli edge nothing pinned, and Maven leaves it
    // compile-reachable: cli depends on render and the export engines, which pull
    // uml/archimate/schema-cache onto its compile classpath. A dev.dediren.uml import in cli —
    // notation logic leaking into the entrypoint, against the §3 thin-cli charter — would have
