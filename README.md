...
```text
dediren-agent-bundle-0.17.0-x86_64-unknown-linux-gnu.tar.gz
dediren-agent-bundle-0.17.0-aarch64-unknown-linux-gnu.tar.gz
dediren-agent-bundle-0.17.0-aarch64-apple-darwin.tar.gz
```
...
`elkrs-core` and `elkrs-layered` are pinned as Git dependencies because they
are not yet published on crates.io.

Omitting `--target` builds for the current supported host target. For the
current `0.17.0` version and the target above, the xtask creates:

```text
dist/dediren-agent-bundle-0.17.0-x86_64-unknown-linux-gnu/
dist/dediren-agent-bundle-0.17.0-x86_64-unknown-linux-gnu.tar.gz
```

Run the smoke test from a shell where `xmllint --version` succeeds. Export
validation also needs either cached standards schemas, configured local schema
paths, or `curl` network access to populate the cache:
...
under `.cache/locks/` because release binaries and `dist/` artifacts are shared
generated outputs.
...
`plugins/`, first-party plugin binaries under `bin/`, schemas, and fixtures.
The archive contains Rust plugin binaries and has no separate ELK helper
runtime or Java prerequisite. The `THIRD-PARTY-NOTICES.md` file is generated by
`cargo-about` for the Rust workspace dependency graph; keep it with
...
- `layout` asks a layout plugin to generate a layout result. The bundled
  `elk-layout` plugin runs ELK Layered in-process through Rust `elkrs`.
...
| `elk-layout` | `layout` | Runs Rust `elkrs` ELK Layered layout and returns generated layout results. |
...
First-party manifests declare only the runtime variables needed by each plugin.
For `elk-layout`, that surface is the deterministic fixture override used by
tests and repair loops.

## ELK Runtime

The first-party `elk-layout` plugin reads a `layout-request.schema.v1` document
from stdin and returns a command envelope whose `.data` is a
`layout-result.schema.v1` document. It uses Rust `elkrs` for ELK Layered
generated node and group placement plus edge routing; source graph JSON still
must not carry authored coordinates. `elkrs-core` and `elkrs-layered` are pinned
as Git dependencies because they are not yet published on crates.io.

`DEDIREN_ELK_RESULT_FIXTURE` remains available for deterministic tests and
repair loops. When set, fixture mode returns the referenced layout result
instead of running the Rust layout backend.

The Rust backend does not require Java, and the distribution archive has no
separate ELK helper runtime.

The layout result contract still supports renderer advice through
`routing_hints` values such as `shared_source_junction` and
`shared_target_junction`, and the SVG renderer honors those hints when present
in fixtures or future backend output. The current Rust `elkrs` backend does not
emit shared junction routing hints yet.

`layout-request.schema.v1` edges may carry an optional `relationship_type`. The
`generic-graph` plugin projects it from each source relationship type, but the
current Rust backend does not use it for endpoint merging yet. If the field is
...
profiles are `compact`, `readable`, and `spacious`. Supported routing style is
`orthogonal`. `wrapping` still accepts `auto`, `off`, and `multi-edge` for
schema compatibility, but the Rust backend reports
`DEDIREN_ELK_OPTION_UNSUPPORTED` for `auto` and `multi-edge` because `elkrs`
does not implement Dediren wrapping preferences yet. `routing.profile` accepts
`compact`, `readable`, and `spacious`, and `elkrs` 1.0.0 applies the
edge-spacing options behind those profiles without backend warnings. Compound
and hierarchy-crossing routes can still produce bounded Dediren route-quality
warnings such as detours, close parallel route channels, or connector-through
node counts. `endpoint_merging` accepts `auto`, `local`, and `off`; `auto` and
`local` currently report `DEDIREN_ELK_OPTION_UNSUPPORTED` and do not emit shared
junction routing hints. Missing preferences keep the backend's default behavior.
...
- Runtime-boundary tests exercise plugin discovery, fixture precedence, and
  error-envelope behavior with test plugins or fixtures.
- `rust_elk_*` tests run the Rust `elkrs` backend. Render evidence tests write
  SVGs but are part of the normal Rust test lane.

The default `cargo test --workspace --locked` lane includes the Rust ELK backend
tests. The render-evidence target can still be run directly when you need only
the generated SVG artifacts.

Generated render artifacts are written under `.test-output/renders/`:

- `.test-output/renders/rust-elk/` for Rust ELK render tests.
...
The Rust ELK render tests also write UML class, data, activity, and complex UML
source-view SVGs to `.test-output/renders/rust-elk/`. The cargo integration
test target is still named `real_elk_render`.
...
Distribution checks from a shell where `xmllint --version` succeeds:
...
Real ELK checks from a source checkout:

```bash
cargo test --locked -p dediren-plugin-elk-layout --test elk_layout_plugin rust_elk_plugin_layouts_basic_request_without_external_runtime -- --exact --test-threads=1
cargo test --locked -p dediren --test cli_layout rust_elk_layout_invokes_in_process_backend -- --exact --test-threads=1
cargo test --locked -p dediren --test cli_layout rust_elk_layout_validates_grouped_cross_group_route -- --exact --test-threads=1
cargo test --locked -p dediren --test real_elk_render -- --test-threads=1
```
