...
`"dot"`, and `output: "svg"` when an image is useful (`"data"` is the
default). Examples are `flowchart LR\n  client --> api` and
`digraph { client -> api }`. SVG uses the bundled default policy unless
`render_policy` is supplied. The same output selector is available to
`dediren_build`, where a source-mode SVG build requires an explicit policy;
see the agent guide for limits, client display support, and failure handling.
