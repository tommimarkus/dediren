...
| [SVG Rendering](svg-render.md) | The SVG render plugin, render policies, interactive modes, and the `artifacts[]` result shape |
...
- **SVG render policy field, interaction mode, or styling behavior** → update
  [SVG Rendering](svg-render.md) alongside `schemas/svg-render-policy.schema.json`.
...
**Version strings.** Pages cite the current product version (`2026.06.6` at the
