...
| Every automated schema-validation lane (unit through dist-smoke) validates OEF/XMI output against permissive stub XSDs, never the official OMG/OpenGroup schemas — the OEF lane in-JVM via `schemacache.InJvmXmlValidator`, the XMI lane via `xmllint` | `engines/{archimate-oef-export,uml-xmi-export}` test resources, dist-smoke | §4 contract surface | real schemas are not redistributable and remote fetch is non-hermetic; "schema-valid export" is therefore proven for the plumbing only. Manual real-schema validation stays possible via `DEDIREN_OEF_SCHEMA_DIR` (the real Open Group set also needs the W3C `xml.xsd` it imports) / `DEDIREN_XMI_SCHEMA_PATH`; the runtime cache lane already fetches the pinned real OEF set, so a hermetic real-schema *test* lane remains the open candidate follow-up (accepted 2026-07) |
