...
| Native free-port ordering replaces Dediren port indexes | `engines/elk-layout/.../ElkLayoutEngine.createEdgePort`; `LayoutQualityFixtureSweepTest` | §7 ELK-first | ELK owns port order under `PortConstraints.FIXED_SIDE`; Dediren supplies only endpoint faces. The former `PORT_INDEX` convention and the two-node-cycle crossing exception are gone. Fixture crossings are zero where the witness graph permits zero; the aggregate metric remains informational for arbitrary graphs. |
| Compact split/rejoin endpoint sides | `engines/elk-layout/.../PortPlan.flatEndpointSides`; `fixtures/source/valid-uml-compact-{sibling-fan-out,side-return}.json`; render/paint acceptance corpus | §7 ELK-first | The compact binary split/rejoin keeps its primary spine and one outer return corridor; compact non-rejoining sibling fan-outs keep primary-flow endpoint faces. This is ELK port intent, not a second router or a broader `OrthogonalRouteNormalizer`. The two focused fixtures are zero-crossing witnesses. The 13 Fivee views are preliminary at zero crossings and clearance at least 24; parent final metrics and rendered review remain pending. |
| Native ELK partitioned self-module crossing residual | `engines/elk-layout` package/self-model input | §7 ELK-first | The baseline has 26 crossings; high thoroughness produces 18 in the package. The exact same partitioned input can yield 17 or 18 because ELK 0.12's `HashMultimap` identity-order allocation changes `PartitionMidprocessor` ordering; this is a localized inference, not a global optimality claim. The exact lower bound of 17 applies only to monotone two-fixed-band routes. The graph is planar, so neither 17 nor 18 is globally unavoidable or optimal. **Final parent evidence: pending regenerated package metrics and Firefox review.** |
