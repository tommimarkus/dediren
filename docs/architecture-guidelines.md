...
| Compact sibling endpoint sides — **resolved 2026-08-21** by ELK port planning | `engines/elk-layout/.../PortPlan.flatEndpointSides`; `fixtures/source/valid-uml-compact-sibling-fan-out.json`; `LayoutQualityFixtureSweepTest` | §7 ELK-first | **keep candidate B.** A compact UML decision's sibling ports formerly alternated from the primary flow face to orthogonal corners. The focused fan-out regression showed the second branch leaving north instead of east; placing every compact sibling endpoint on the primary flow side, while retaining ELK's existing `CONSIDER_MODEL_ORDER_PORT_MODEL_ORDER = true`, gives two east exits with zero near-source crossings and no excessive detour. Candidate C (disable port model order) and candidate D (restore alternation only for `junction`) were not selected: the only residual corpus crossing is the corrected metric's pre-existing state-machine baseline. This is port intent only: do not add a route post-processor or broaden `OrthogonalRouteNormalizer`; the new layout-result fixture itself is held at zero by the sweep, while the unrelated state-machine baseline is explicitly pinned at one. |
