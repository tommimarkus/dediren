...
| `drawio` (engine) | `engine-api`, `contracts`, `ir` (`RouteGeometry` only) | 2 — leaf engine |
| `archimate-oef-export` (engine) | `engine-api`, `contracts`, `archimate`, `schema-cache`, `ir` (`RouteGeometry` only) | 2 — leaf engine |
| `uml-xmi-export` (engine) | `engine-api`, `contracts`, `uml`, `schema-cache`, `ir` (`RouteGeometry` only) | 2 — leaf engine |
...
- **`engine-api` speaks `ir` types (Plan B P4 seam flip); export stays on
  ...
  writing and re-reading `layout-result.json`. Format engines may call the pure
  shared `ir.RouteGeometry` operations for bounded waypoint approximation; the
  ArchUnit rule still forbids every other IR class from those engines.
...
| Native node sizing — supersedes the July fixed-side trial | `ElkLayoutEngine.setGeneratedDimensions`, `PortPlan` | §7 ELK-first | Ordinary FREE/JUSTIFIED ports use ELK `minimumSizeWithPorts()` with authored/default minima and the surrounding margin. The approved readability-first routing change retires custom port-count growth. Sequence and compact control connectors retain their explicit shape/port intent; their size and face regressions remain pinned. |
| Compound-edge route-section normalization — retained narrowly | `engines/elk-layout/.../OrthogonalRouteNormalizer.java` | §7 ELK-first | Only the existing alternating six-point orthogonal join candidate remains. Source-boundary pivots and general four-point dogleg rewrites are retired. Loops, curves and non-orthogonal geometry bypass it; endpoint approach, node/group clearance, crossings and shared-junction guards reject unsafe candidates. Uncertainty preserves native geometry. |
...
| Native free-port ordering | `ElkLayoutEngine.createEdgePort`; `LayoutQualityFixtureSweepTest` | §7 ELK-first | Ordinary nodes use FREE ports, with native side/order selection. FIXED_SIDE is confined to compact controls and sequence intent. All 21 layout fixtures have zero crossings; arbitrary-graph crossings remain informational. |
| Compact split/rejoin endpoint sides | `PortPlan`; compact render/paint fixtures | §7 ELK-first | Compact binary controls retain their primary spine and outer return corridor. All 13 campaign views have zero crossings and at least 24 units of unrelated-node clearance after actual Firefox review. See [routing acceptance](features/routing-acceptance.md). |
| Native ELK partitioned self-module crossing residual | `engines/elk-layout`; self-model module view | §7 ELK-first | Reviewed package: 26 → 18 crossings. Identical partitioned input can produce 17 or 18 after intervening allocations; pinned ELK identity-based partition ordering is the localized inferred cause. The 17 lower bound applies only to monotone two-fixed-band routes, not global planarity or optimality. Every retained event and its scope-limited justification is listed in [routing acceptance](features/routing-acceptance.md). |
...
| Inferred perpendicular group axes — retired | `ElkLayeredOptions.configureGroup`; `PortPlan.grouped` | §7 ELK-first | All INCLUDE_CHILDREN groups now inherit the requested root direction. Native ELK owns ports and cycle breaking; the former inferred-axis and grouped pre-reversal paths are removed. The former trial-specific body-crossing rationale no longer describes the current implementation. |
| Node-face hugging — guarded and measured | `OrthogonalRouteNormalizer`; `LayoutQuality`; native regressions | §7 ELK-first | The source-boundary pivot is removed; remaining candidates must preserve clearance. Quality detects unrelated-node proximity below 24 units and own-node re-entry. Feasible fixtures have zero clearance warnings; native hierarchical behavior remains subject to independent geometry and rendered acceptance, without a second routing pass. |
...
| Debt | Location | Guideline | Smell |
|---|---|---|---|
| `ElkLayoutEngine` orchestration size — narrowed by routing cutover | `ElkLayoutEngine`, `LayoutRequestValidator`, `PortPlan`, `ElkLayeredOptions` | §8 thinness/cohesion | Request validation and native option configuration are extracted. Group-direction inference and pre-reversal are retired; explicit partitions and constraints have regressions. The engine retains graph construction, extraction and sequence orchestration. Further extraction should follow a concrete cohesion need. |
