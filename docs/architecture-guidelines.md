...
| Debt | Location | Guideline | Smell |
|---|---|---|---|
| ArchiMate endpoint legality accepts roughly a quarter of the combinations Appendix B forbids, by design: the rules are expressed over the generic metamodel's element categories rather than by reproducing the copyrighted B.5 tables, and no derivation closure is computed | `archimate/.../RelationshipLegality.java` | §4 contract surface | accepted as the IP-clean posture (see the 2026-07 legality restoration); **trigger:** if `dediren validate` is ever presented as a conformance gate rather than an early check, or if the catch rate measured against a local Appendix-B oracle falls below the recorded ~78%, revisit — the disclosure in `docs/agent-usage.md § ArchiMate Handoff` is what makes the posture honest and must move with any change |
...
| ArchiMate composition/aggregation legality is decided on element *category* rather than element type, and `Product`/`Plateau` share the composite category, so some cross-layer containment passes; §B.4's four domains are not modelled at all | `archimate/.../RelationshipLegality.java` | §4 contract surface | `AM-SEM-3/-4/-7`; **trigger:** needs the layer/domain dimension the model does not carry — and narrowing it without the Appendix-B oracle in hand risks the false rejections the class promises never to make. Tracked as followup `F3` |
