...
- **A dependency belongs to the engine that needs it.** An engine reports its
  ...
  an export engine emits `DEDIREN_OEF_SCHEMA_UNAVAILABLE` /
  `DEDIREN_XMI_SCHEMA_UNAVAILABLE` when its schema must be fetched and `curl`
  is absent; both export lanes validate in-JVM). Keep
...
| Layout-quality metric math re-implemented in the ELK plugin's e2e tests | `engines/elk-layout/.../ElkLayoutEngineTest.java`, `ElkLayoutLaneAgreementTest.java`, `ElkLayoutInvariantFuzzTest.java` (each carries its own geometry-metric helpers) vs `core/quality/LayoutQuality.java` | §2 no plugin→`core` edge | `LA-CODE-DUP-2` (lean-audit 2026-07-06), accepted: plugins may depend only on `contracts` (the sole test-scope exception belongs to `cli`), and `test-support` cannot host `contracts`-typed helpers without a reactor cycle (`contracts` test-depends on `test-support`); the independent copies deliberately corroborate core's quality metrics against real ELK output; the two new lane-agreement and invariant-fuzz test classes (added with the `PortPlan` extraction) carry the same boundary reason as the original `ElkLayoutEngineTest` helpers; marked `lean-audit:dup-intentional`; revisit if a contracts-aware test-support home is ever chartered |
...
| Compound-edge route-section normalization is custom post-ELK geometry — **trialled and retained 2026-08-12** | `engines/elk-layout/.../OrthogonalRouteNormalizer.java` | §7 ELK-first | **keep narrowly, with evidence.** ELK Layered 0.12.0 natively owns the compound nodes and cross-hierarchy edge sections, but joining those sections produced five-segment alternating stairs plus residual 1.33px and 11px doglegs in `valid-pipeline-rich.json`. Trials covered `favorStraightEdges`, balanced/BK/Network Simplex/Linear Segments/Simple placement, root and node port alignments, `FIXED_SIDE`, zero-sized/native ports, hierarchy sweepiness, hierarchy-edge merging, feedback-edge handling, and flat decorative groups with partitioned, unpartitioned, wrapped, and downward layouts. None removed the artifacts without worse corners, detours, overlapping/misleading group boxes, label displacement, or loss of the requested reading direction. The retained pure normalizer changes only (a) an alternating six-point orthogonal section join or (b) a four-point dogleg whose cross-axis adjustment stays within its source node's span, and only when a shorter or equal collision-free route exists. It preserves ELK's endpoints and target approach; the fewest-turn source-boundary pivot is reserved for a whole six-point route, while a join inside a longer route may use only ELK's existing channels. That restriction was added after extending an internal channel backward created a new close-parallel quality warning. The source pivot is also disabled for merged source endpoints because a broad application erased ELK's deliberate shared fan-out trunk. `OrthogonalRouteNormalizerTest` pins whole-route scope, source-span, collision fallback, shared-trunk, and preservation cases; `ElkLayoutEngineTest.groupedPipelineHasNoVisibleStairing` and `groupedGatewayFanOutUsesMergedJunctionRoute` pin the real-engine outcomes. Re-litigate against a newer ELK release before broadening the normalizer or adding any second routing pass. |
...
| UMLDI (UML Diagram Interchange) export ships a **provisional** OMG DD/DI dialect never verified against a real UML tool's importer, and — like the UML namespace — has no normative OMG DI XSD, so its `di:`/`dc:`/`umldi:` content rides the tolerated no-declaration gap rather than being schema-validated | `engines/uml-xmi-export/.../write/diagram/DiagramWriter.java` (dialect namespace constants), `.../schema/SchemaValidation.java` (tolerated prefixes) | §4 contract surface | accepted 2026-07: the Papyrus/Sparx-EA **render** probe the roadmap survey §3.3 mandates before commitment was substituted, at the user's direction, with an executable structural/geometry conformance gate (well-formed, `xmi:id`-unique, referential integrity, geometry fidelity); **trigger:** run the Papyrus + EA import probe before broadening DI beyond the class family (`uml-class`/`uml-data`) or treating the dialect as stable — a wrong dialect is a one-place fix in `DiagramWriter`'s namespace constants, and the DI output is contained to the opt-in whole-model `model.uml.xml` aggregate (per-view `xmi.xml` stays model-only) |
| Edge port index convention — **resolved 2026-08-17** by migration to ELK-native ordering | `engines/elk-layout/.../ElkLayoutEngine.createEdgePort` (now uses `PortConstraints.FIXED_SIDE`, deletes `PORT_INDEX` assignment); `LayoutQualityFixtureSweepTest.EXPECTED_EDGE_CROSSING_COUNTS` | §7 ELK-first | **deleted, with evidence** — the inverse of the port-sizing row above: there the ELK option could not express the intent and the custom code stayed; here it could, and the custom code went. ELK's `PORT_INDEX` under `PortConstraints.FIXED_ORDER` encodes a clockwise order from the top-left corner (NORTH left→right, EAST top→bottom, SOUTH right→left, WEST bottom→top). `ElkLayoutEngine` handed out ascending indices in edge-declaration order on every side, so the two reversed sides (WEST, SOUTH) came out inverted. Under `Direction.RIGHT`, WEST is the target side of every edge, thus all target ports were backwards. Three divergent conventions coexisted: flat lane's ascending `nextPortIndex`, grouped lane's WEST-only descending `reverseGroupedPortIndex`, and merge ports' `nextExistingPortIndex` returning `maxIndex + 1`. This closes findings #2 and #4 of the 2026-07-24 elk-layout geometry audit by deletion. Resolution: `createEdgePort` now sets `PortConstraints.FIXED_SIDE` and writes no `PORT_INDEX`; Dediren keeps only the port side (sources EAST, targets WEST) and ELK owns ordering within a side via `LayeredOptions.PORT_SORTING_STRATEGY` and `CONSIDER_MODEL_ORDER_PORT_MODEL_ORDER`, both left as-is. Evidence: edge crossings went **9→0** across the checked-in layout-result corpus—`uml-complex-class` alone 6→0, `pipeline-rich` 1→0, `uml-component-basic` 1→0, `uml-deployment-basic` 1→0; `uml-complex-class` shrank ~39% in area; `LayoutQualityFixtureSweepTest`'s `EXPECTED_EDGE_CROSSING_COUNTS` is now empty. A hand-corrected sign convention was trialled first and produced byte-identical geometry to `FIXED_SIDE` on `Direction.RIGHT` and `Direction.DOWN`, so the ELK-native option was preferred per §7. This supersedes only the `FIXED_SIDE` mention in the existing `OrthogonalRouteNormalizer` row—that trial addressed compound-hierarchy stair artifacts and is unaffected. Flipping `CONSIDER_MODEL_ORDER_PORT_MODEL_ORDER` to `false` was probed and produced byte-identical fixtures, so it remains currently inert on this corpus. |
| Compact split/rejoin endpoint sides — **resolved 2026-08-21** by ELK port planning | `engines/elk-layout/.../PortPlan.flatEndpointSides`; `fixtures/source/valid-uml-compact-{sibling-fan-out,side-return}.json`; `LayoutQualityFixtureSweepTest` | §7 ELK-first | A compact binary split that reconverges through two linear branches now pairs each source branch with its terminal incoming edge: branch 0 stays on the primary flow face and branch 1 uses the same orthogonal face at both ends, giving one straight spine plus one outer return corridor. Compact fan-outs that do not reconverge, and fan-ins/out with other degrees, keep every sibling endpoint on the primary flow face so ELK owns their port order. The rejected all-primary variant removed crossings but introduced five-segment excessive detours in `outside-your-turn`, `material-adaptation`, and `character-advancement`; blind alternation caused the original near-merge crossings because incoming declaration order did not identify the matching source branch. The focused fixtures and the 13-view Fivee gallery both hold at zero proper crossings, with no new detours. This remains port intent only: do not add a route post-processor or broaden `OrthogonalRouteNormalizer`; the unrelated state-machine crossing baseline remains explicitly pinned at one. |
| Sequence view Y-origin depends on ELK-computed translation, not canonical margin | `engines/elk-layout/.../LayoutIntentNormalizer.java` (rebuilds lifeline lattice but has no re-origin step); `fixtures/layout-result/uml-sequence-*.json` (y-coordinate varies with graph shape); `SequenceSelfMessageHookTest` (pinned `d=` path literal) | §7 ELK-first | `LayoutIntentNormalizer` rebuilds the sequence lifeline lattice but has no re-origin step, so a sequence view inherits ELK's raw Y origin, and whole-diagram top margin becomes an artifact of graph shape rather than canonical. Evidence: `fixtures/layout-result/uml-sequence-self-message.json` sat at `y=292` where every other fixture starts at `y=12`; the port-constraint change moved it to `12` and moved `uml-sequence-fragments.json` from `12` to `108`. Shape, spacing, and x are untouched—the drift is pure translation either way, arbitrary rather than directionally wrong. Cost when it moves: silently re-baselines sequence layout fixtures, their render goldens, and the pinned `d=` path literal in `SequenceSelfMessageHookTest`. **Trigger:** normalize the lattice to the standard 12px margin on the next substantive change to `LayoutIntentNormalizer`, so sequence geometry stops depending on ELK's raw translation. |
| Residual perpendicular-group-axis body crossings — **trialled and rejected 2026-08-17** | `engines/elk-layout/.../PortPlan.grouped` / `alongLayerAxis` | §7 ELK-first | **keep the current variant, with evidence.** A wider sweep of 600 generated acyclic grouped graphs found 28 still route a segment through a node body, all where the group's `internalDirection` is perpendicular to the root direction. A variant that pins the intra-group port axis to the root direction unconditionally — rather than only correcting the anti-parallel case — removes every measured body crossing (0 vs 28 interior), but fails two tests that pin deliberate reading-direction intent (`ElkLayoutEngineTest.groupedConnectorEdgesKeepHorizontalFlowInsideVerticalGroups`, `groupedPipelineKeepsReadableLeftToRightFlow`) and is worse on ride metrics (18/100 vs 15/100 fuzz cases, 809 vs 778 of 1500 sweep cases). **Decision recorded by the maintainer: keep the current variant — overruling pinned reading-direction intent is a product call, not a bug fix.** Mechanism: a nested compound node's own `CoreOptions.DIRECTION` does not move layers under `HIERARCHY_HANDLING = INCLUDE_CHILDREN` — a 3-node chain in a `DOWN` group under a `RIGHT` root lays out at x=36/236/436 (layering follows the root) while y drifts 36/166/296, a diagonal staircase caused purely by ports pinned on the group's own axis. **Trigger:** revisit if the perpendicular-axis case shows up in a real view, or if the pinned reading-direction intent above is ever deliberately retired. |
| ELK compound-node edge/face spacing rides ~1px off a member's face inside a group, ignoring the configured 24px `SPACING_EDGE_NODE` | `engines/elk-layout` (native ELK hierarchy-crossing routing; no Dediren post-processing touches this) | §7 ELK-first | measured example: a segment `(708,110.5)`→`(708,181)` runs parallel to and 1px off the left face of a node at `(709,66)` sized 150×72, riding for 27.5px. Reproduces on a strictly ACYCLIC grouped graph and byte-identically on the unmodified baseline, so it is **not** a back-edge defect and **not** the `OrthogonalRouteNormalizer` row above's concern — that normalizer only rejoins ELK's own section geometry and does not touch this face-hugging behavior, so do not "fix" this by broadening that already-litigated normalizer. Sole cause of all 15 residual fuzz cases; pinned as a ceiling in `ElkLayoutInvariantFuzzTest`. §7 ELK-first: the first move on this row is to find the ELK option or graph-structure cause, not add a post-process route pass. |
...
| Debt | Location | Guideline | Smell |
|---|---|---|---|
| `ElkLayoutEngine` is a re-forming god class — **narrowed 2026-08-17**: port planning (endpoint sides, endpoint-merge policy, port counts, back-edge reversal) is now `PortPlan`, so the class sheds direction inference and index state too; what remains is request validation, the four per-mode orchestrations, group construction (`createElkGroup`/`internalDirection`/`configureGroup`, still keyed on the file-local `groupDirectionById`), result extraction, and sequence special-casing beyond the port sides `PortPlan.sequence` now owns | `engines/elk-layout/.../ElkLayoutEngine.java` (1,716→1,252 LOC, -568/+52 diff) vs the new `engines/elk-layout/.../PortPlan.java` | §8 thinness/cohesion | `SD-B-1`; **trigger:** on the next substantive change, extract a `LayoutRequestValidator` (the validation cluster is already self-contained); the four per-mode orchestrators, group construction, and result extraction may stay. **Unmodelled interaction found in passing:** `layoutGrouped` still calls `activatePartitioning`, and ELK's `PARTITIONING_PARTITION` is a stronger layer constraint than the group-declaration rank `PortPlan`'s reversal set uses — the two could disagree, but no test in the repo combines grouped layout with `LayoutNode.partition()` and the fuzz suite never sets it |
...
The 2026-08-12 notation-conformance register
(`docs/superpowers/plans/2026-08-12-notation-conformance-register.md`, remediated
by the plan beside it) measured the product against UML 2.5.1 and ArchiMate 3.2.
Its `block` and most `warn` findings were fixed; the rows below are what was
deliberately accepted, each with the condition that should reopen it. Findings
whose fix needs a surface that does not exist yet are not here — they are the
`## Post-plan followups` table in the remediation plan, which names the
prerequisite rather than the finding.

| Debt | Location | Guideline | Smell |
|---|---|---|---|
| ArchiMate endpoint legality accepts ~12% of the combinations Appendix B forbids, by design: the rules are expressed over the generic metamodel's categories and §B.4 domains rather than by reproducing the copyrighted B.5 tables, and no derivation closure is computed | `archimate/.../RelationshipLegality.java` | §4 contract surface | accepted as the IP-clean posture; **trigger:** if `dediren validate` is ever presented as a conformance gate rather than an early check, or if the measured catch rate falls below the recorded 88.2%, revisit — the disclosure in `docs/agent-usage.md § ArchiMate Handoff` is what makes the posture honest and must move with any change |
| `CATCH_RATE_FLOOR = 0.75` gates the opt-in ArchiMate oracle conformance test, and its only measured basis is ~78% at the time the rules were written. The test is `assumeTrue`-skipped without a user-supplied oracle file, so in an ordinary build the constant gates nothing | `archimate/src/test/.../ArchimateRelationshipLegalityConformanceTest.java` | §4 contract surface | the floor is a regression backstop, not a target; **trigger:** re-measure whenever the rule model changes and raise the floor to just under the measured rate, so erosion between 78% and 75.1% cannot pass unnoticed as it can today |
| ArchiMate containment legality is decided on element *category* rather than element type, so same-category containment inside one domain passes where the tables forbid it (a business actor composing an application component) | `archimate/.../RelationshipLegality.java` | §4 contract surface | remainder of `AM-SEM-3` after the §B.4 domain dimension landed 2026-08-12; **trigger:** tightening to element-type equality is *not* the fix — it would reject a business process composed of business functions. Needs a layer dimension below domain, verified with `-Ddediren.archimate-oracle` so a false rejection cannot land unnoticed |
| UML `local`/`external` transition kinds, the fork/join segment guard rules, and the entry/exit/terminate pseudostate degree constraints are unenforced because composite States are not expressible: a `Region` names a `StateMachine` and never a `State` | `uml/.../Uml.java` | §4 contract surface | accepted 2026-08-12 — approximating a constraint whose subject cannot be modelled would reject legal models; **trigger:** if composite states gain a source surface, these three clauses land with it |
| Six UML edge kinds (`Usage`, `Dependency`, `Include`, `Extend`, `Manifestation`, `Deployment`) render byte-identically, because the guillemet keyword that distinguishes them has no render surface | `engines/render/.../EdgeRenderer.java` | §7 ELK-first | `UML-NOT-1`; attempted 2026-08-12 and reverted — composing the keyword at render time widens a label layout has already reserved space for, which the paint oracle caught. **Trigger:** compose the keyword in projection, upstream of layout, so the reserved width is correct |
| UML `Association` ends are emitted non-navigable in both directions — an assertion, not an omission — and `out`/`inout` parameters export as `in` | `engines/uml-xmi-export/.../write/classifier/` | §4 contract surface | `UML-XMI-10`, `UML-VOCAB-2`; both need an authored surface that does not exist; **trigger:** add navigability and parameter direction to the source contract together, since both are breaking `model.schema.json` additions |
| The UML/XMI coverage partition is id-presence only, so an element exported with the wrong metaclass still counts as represented and attribute-level loss is invisible | `engines/uml-xmi-export/.../UmlXmiAssurance.java` | §4 contract surface | `UML-XMI-17`; **trigger:** making `Coverage` metaclass-aware changes a released contract, so it belongs with the next assurance schema version rather than alone |
| The UMLDI `dc:`/`di:` namespaces and element naming were suspected wrong (`UML-XMI-14`, `UML-XMI-15`) and are **not** — checked 2026-08-12 against the OMG's published DD schemas. `DC.xsd` declares `Bounds` as a global element named after its type; `waypoint` is a local element in `DI.xsd`, which sets `elementFormDefault="qualified"`; and `20100524` is those schemas' `targetNamespace`, not a stale DD 1.0 reference. DD 1.1's `20131001` stamps metamodel XMI files only | `engines/uml-xmi-export/.../write/diagram/DiagramWriter.java`, pinned by `DiagramWriterConformanceTest` | §4 contract surface | no debt — recorded as a **do-not-change**: "upgrading" to `20131001` would emit documents no DD-aware tool can read. What is still open is the render probe in the row above, which is a different question |
