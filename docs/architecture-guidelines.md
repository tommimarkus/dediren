...
| SpotBugs `EI_EXPOSE_REP`/`EI_EXPOSE_REP2` suppressed on the styling records that carry an optional `dash_pattern` (`contracts.render.Svg{Node,Group,Edge}Style` and `render/style/Resolved{Node,Group,Edge}Style`) | `spotbugs-exclude.xml` | §6 contract surface / §8 thinness | deferred: each wraps its `List<Double> dashPattern` via `ContractCollections.copyOrNull` (`List.copyOf`, null-preserving so an unset override differs from an empty list), so sharing the reference is genuinely immutable; suppressed rather than defensive-copied for consistency with the existing records (broaden-styling: lines & strokes) |
