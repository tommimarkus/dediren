...
**Tech Stack:** Java 21, Maven Wrapper (`./mvnw`), JUnit 5 (Jupiter), AssertJ, Jackson (`tools.jackson`), networknt json-schema-validator via `dev.dediren.core.schema.SchemaValidator`, `java.security.MessageDigest` (SHA-256), `java.nio.file` POSIX file attributes. No Mockito — process-boundary plugins run for real in the runtime tests.
