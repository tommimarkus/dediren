 ...
 * {@code test-support} must not depend on {@code contracts} (which depends on {@code test-support}
 * at test scope), so it cannot use the contracts {@code JsonSupport} mapper.
 */
public final class CommandEnvelopeAssertions {
  ...
  /**
   * Asserts the envelope status is {@code error} and its first diagnostic code equals {@code
   * expectedCode}.
   */
