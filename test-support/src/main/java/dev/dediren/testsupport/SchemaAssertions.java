...
public final class SchemaAssertions {
  ...
  public static List<String> validateFixture(
      Path repositoryRoot, String schemaPath, String fixturePath) {
    try {
  ...
  public static List<String> validate(Path repositoryRoot, String schemaPath, JsonNode data) {
    try {
      ...
      return schema.validate(OBJECT_MAPPER.writeValueAsString(data), InputFormat.JSON).stream()
  ...
  public static void assertSchemaValid(Path repositoryRoot, String schemaPath, JsonNode data) {
    assertThat(validate(repositoryRoot, schemaPath, data)).describedAs(schemaPath).isEmpty();
  }
