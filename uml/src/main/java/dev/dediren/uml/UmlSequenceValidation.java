...
/**
 * UML interaction/sequence validation: combined fragments, interaction
 * operands, message sequencing, and fragment coverage/nesting/ownership.
 * Extracted verbatim from {@link Uml} to keep the facade small; behavior,
 * diagnostic codes, and JSON paths are unchanged.
 */
final class UmlSequenceValidation {
    ...
    static void validateCombinedFragmentNesting(
            List<SourceNode> nodes,
            ValidationContext context) throws UmlValidationException {
        for (SourceNode node : nodes) {
