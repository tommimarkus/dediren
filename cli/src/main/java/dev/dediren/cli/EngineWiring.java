...
/**
 * The single, explicit binding of the bundled first-party engines into an {@link Engines} registry
 * (decision 3). The semantics slot is the profile router ({@link SemanticsRouterEngine}) over the
 * per-notation {@link NotationSemantics} implementations, rather than the monolithic generic-graph
 * engine, but it still publishes engine id {@code "generic-graph"}. This is the only cli class
 * permitted to reference {@code dev.dediren.plugins..} and {@code dev.dediren.semantics..}
 * implementations — every other cli class knows engines only through the {@code engine-api}
 * interfaces, a boundary the dist-tool {@code ArchitectureRulesTest} enforces. A hardcoded list of
 * in-tree engines needs no lookup indirection and gives ArchUnit a single named class to confine
 * the cli to engine-implementation edge to.
 */
